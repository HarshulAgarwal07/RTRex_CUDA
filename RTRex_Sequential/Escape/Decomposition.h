...
char* initialClean(CGraph* g, WeightedTriangleInfo* triInfo, double eps, bool use_parallel_seed = false)
{
    ...
    if (use_parallel_seed)
    {
        int nthreads = omp_get_max_threads();
        vector< vector< pair<EdgeIdx, Pair> > > local_seed_edges(nthreads);

        #pragma omp parallel
        {
            int tid = omp_get_thread_num();
            vector< pair<EdgeIdx, Pair> >& seed_edges = local_seed_edges[tid];

            #pragma omp for schedule(dynamic, 64)
            for(VertexIdx u=0; u < g->nVertices; u++)
            {
                Count degu_local = g->offsets[u+1] - g->offsets[u]; // get the degree of u
                for (EdgeIdx i = g->offsets[u]; i < g->offsets[u+1]; ++i)   // loop over neighbor of u
                {
                    ...
                    Count degv_local = g->offsets[v+1] - g->offsets[v]; // get the degree of v
                    double edgeWt = 1/double(degu_local*degv_local); // calculate the weight of the edge
                    if (triInfo->perEdge[i] < eps*edgeWt)
                    {
                        Pair edge;
                        edge.first = u;
                        edge.second = v;
                        seed_edges.push_back(make_pair(i, edge));
                    }
                }
            }
        }

        vector< pair<EdgeIdx, Pair> > seed_edges;
        size_t total_seed_edges = 0;
        for (int t = 0; t < nthreads; ++t)
            total_seed_edges += local_seed_edges[t].size();
        seed_edges.reserve(total_seed_edges);

        for (int t = 0; t < nthreads; ++t)
            seed_edges.insert(seed_edges.end(), local_seed_edges[t].begin(), local_seed_edges[t].end());

        sort(seed_edges.begin(), seed_edges.end(), [](const pair<EdgeIdx, Pair>& left, const pair<EdgeIdx, Pair>& right) {
            return left.first < right.first;
        });

        for (auto iter = begin(seed_edges); iter != end(seed_edges); ++iter)
        {
            EdgeIdx loc = iter->first;
            if (edgeStatus[loc] == 'S')
                continue;
            toDelete.push(iter->second);
            edgeStatus[loc] = 'S';
            edgeStatus[g->partnerMap[loc]] = 'S';
        }
    }
    else
    {
...
vector<Cluster> disjointExtract(CGraph* g, double eps, std::map<VertexIdx, VertexIdx>& cluster_map, bool use_parallel_inner = false)
{
    ...
    char* edgeStatus = initialClean(g, &triInfo, eps, use_parallel_inner); // applying the initial clean, to get the current edgeStatus. This array has a 'Y' for every existing edge, and an 'N' for deleted edge. No other symbol should be present
...
//         printf("Seed degree %" PRId64 "\n",degu);
        ...
        if (use_parallel_inner && degu > 64)
        {
            int nthreads = omp_get_max_threads();
            vector< map<VertexIdx,double> > local_triwgt(nthreads);
            vector< vector<VertexIdx> > local_two_hop(nthreads);
            double internalWgt_local = 0.0;

            #pragma omp parallel reduction(+ : internalWgt_local)
            {
                int tid = omp_get_thread_num();
                map<VertexIdx,double>& tri_map = local_triwgt[tid];
                vector<VertexIdx>& two_vec = local_two_hop[tid];
                ...
                for (EdgeIdx j=g->offsets[u]; j < g->offsets[u+1]; ++j)
                {
                    ...
                    for (EdgeIdx k=j; k < g->offsets[u+1]; ++k)
                    {
                        ...
                        internalWgt_local += 1.0/double(degu*degv*degw);
                        ...
                        for (EdgeIdx ell = g->offsets[lower]; ell < g->offsets[lower+1]; ell++)
                        {
                            ...
                            {
                                if (x > v && x > w)
                                    internalWgt_local += wgt;
                                continue;
                            }

                            auto it = tri_map.find(x);
                            if (it == tri_map.end())
                            {
                                tri_map[x] = wgt;
                                two_vec.push_back(x);
            ...
            internalWgt += internalWgt_local;

            for (int t = 0; t < nthreads; ++t)
            {
                for (auto iter = begin(local_triwgt[t]); iter != end(local_triwgt[t]); ++iter)
                    triwgt[iter->first] += iter->second;
                for (VertexIdx x : local_two_hop[t])
                    two_hop.insert(x);
            }
        }
        else
        {
            for (EdgeIdx j=g->offsets[u]; j < g->offsets[u+1]; ++j) // looping over neighbors of u
            {
                ...
                for (EdgeIdx k=j; k < g->offsets[u+1]; ++k) // looping over neighbors of v
                {
                    ...
                    internalWgt += 1.0/double(degu*degv*degw); // add the triangle weight to the internal weight
                    ...
                    for (EdgeIdx ell = g->offsets[lower]; ell < g->offsets[lower+1]; ell++)
                    {
                        ...
                        EdgeIdx loc_hix = g->getEdgeBinary(higher,x); //
                        if(loc_hix == -1 || edgeStatus[loc_hix] == 'N') // if edge is not present, or has been deleted
                            continue;
                        ...
                        double wgt = 1.0/double(degx*degv*degw); // the weight of the triangle
...
//         printf("Cluster size %" PRId64 "\n",next_clus.vertices.size());
    ...
    // ==================== TIMING: End extraction phase ====================
    auto extraction_end = std::chrono::high_resolution_clock::now();
    double extraction_time = chrono::duration<double>(extraction_end - extraction_start).count();

    stop = std::chrono::high_resolution_clock::now();
    cout << "Time for getting the clusters: " << chrono::duration<double>(stop - start).count() << "s \n";
    cout << "\n[EXTRACTION TIMING]\n";
    cout << "Extraction mode: " << (use_parallel_inner ? "parallel-initial-clean+inner-omp" : "sequential") << "\n";
    cout << "Total clusters extracted: " << n_clusters << "\n";
    cout << "Extraction phase time (disjointExtract loop): " << extraction_time << "s\n\n";
