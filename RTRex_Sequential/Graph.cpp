 ...
 CGraph Escape::makeCSR(Graph g, bool inPlace)
{
  ...
  // =================================================================
  // MAC COMPILER FIX: Bypass JSIterator and use std::vector to sort
  // =================================================================
  std::vector<std::pair<VertexIdx, VertexIdx>> temp_edges(tmpG.nEdges);
  for (EdgeIdx i = 0; i < tmpG.nEdges; ++i) {
      temp_edges[i] = {tmpG.srcs[i], tmpG.dsts[i]};
  }
  
  std::sort(temp_edges.begin(), temp_edges.end());
  
  for (EdgeIdx i = 0; i < tmpG.nEdges; ++i) {
      tmpG.srcs[i] = temp_edges[i].first;
      tmpG.dsts[i] = temp_edges[i].second;
  }
  // =================================================================
